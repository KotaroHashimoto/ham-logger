...
### 設定の書き換え
#### ホイール内径の変更
スクリプト main.py の 564行目で、Counter() の引数で指定している数値を、ホイール内径[m]の数値に書き換えます。例えば21cmのホイールへ設定変更する場合は0.21に変更します。  
...
Raspberry Pi Pico 上でスクリプトを書き換え上書き保存すると次回起動時に変更が反映されます。

#### ハムスターの名前の変更
スクリプト main.py の 573行目で、Environment() の引数で指定している文字列を変更します。使用できる文字はアルファベット大文字小文字と数字のみです。  
...
#### ログファイルについて
一度起動すると、以下のようにmain.pyの他に３つのlogファイルが自動で生成されます。これらは電源OFF->ON時に再度読み込まれて前回までの記録を再表示するために使用されます。  
